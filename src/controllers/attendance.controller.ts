...
import Attendance from '../models/attendance.model'; // Importation du modèle Attendance
...
export const createAttendance = async (req: Request, res: Response) => {
    try {
        ...
        res.status(201).json({ message: 'Présence enregistrée avec succès', attendance }); // Réponse avec succès
    } catch (error) {
        res.status(500).json({ message: 'Erreur lors de l\'enregistrement de la présence', error }); // Gestion des erreurs
    }
};
...
export const updateAttendance = async (req: Request, res: Response) => {
    ...
    try {
        const attendance = await Attendance.findByIdAndUpdate(id, req.body, { new: true }); // Mise à jour de la présence
        if (!attendance) {
        ...
        res.status(500).json({ message: 'Erreur lors de la mise à jour de la présence', error }); // Gestion des erreurs
    }
};
...
export const getAttendanceByTimeTable = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Erreur lors de la récupération des présences', error }); // Gestion des erreurs
    }
};
...
export const exportAttendanceToCSV = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const records = await Promise.all(attendances.map(async (attendance) => {
            ...
            return {
                student_name: student ? `${student.firstname} ${student.lastname}` : 'Inconnu', // Récupération du nom complet de l'étudiant
                teacher_name: teacher ? `${teacher.firstname} ${teacher.lastname}` : 'Inconnu', // Récupération du nom complet de l'enseignant
        ...
        res.download('attendance.csv', 'attendance.csv', (err) => { // Téléchargement du fichier CSV
            if (err) {
                res.status(500).json({ message: 'Erreur lors du téléchargement du fichier CSV', err }); // Gestion des erreurs de téléchargement
            }
        });
    } catch (error) {
        res.status(500).json({ message: 'Erreur lors de l\'exportation des présences', error }); // Gestion des erreurs
    }
};
...
export const getAttendanceByUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user.role === 'student') {
            ...
            return res.status(400).json({ message: 'Rôle d\'utilisateur non supporté' }); // Gestion des erreurs pour les rôles non supportés
        }

        res.json(attendances); // Réponse avec la liste des présences
    } catch (error) {
        res.status(500).json({ message: 'Erreur lors de la récupération des présences de l\'utilisateur', error }); // Gestion des erreurs
    }
};
