...
import ExamSchedule from '../models/exam_schedule.model';
...
export const getAllExamSchedules = async (req: Request, res: Response) => {
    try {
        const examSchedules = await ExamSchedule.find().populate('exam_id').populate('classroom_id');
...
export const getExamScheduleById = async (req: Request, res: Response) => {
    try {
        const examSchedule = await ExamSchedule.findById(req.params.id).populate('exam_id').populate('classroom_id');
        if (!examSchedule) return res.status(404).json({ message: 'Exam Schedule not found' });
...
export const updateExamSchedule = async (req: Request, res: Response) => {
    try {
        ...
        const updatedExamSchedule = await ExamSchedule.findByIdAndUpdate(req.params.id, { exam_id, classe_id }, { new: true });
        if (!updatedExamSchedule) return res.status(404).json({ message: 'Exam Schedule not found' });
...
export const deleteExamSchedule = async (req: Request, res: Response) => {
    try {
        const deletedExamSchedule = await ExamSchedule.findByIdAndDelete(req.params.id);
        if (!deletedExamSchedule) return res.status(404).json({ message: 'Exam Schedule not found' });
