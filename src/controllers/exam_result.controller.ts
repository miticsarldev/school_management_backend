...
export const createExamResult = async (req: Request, res: Response) => {
    try {
        const { exam_id, student_id, course_id, grade, comments, status } = req.body;
...
export const getAllExamResults = async (req: Request, res: Response) => {
    try {
        const examResults = await ExamResult.find().populate('exam_id').populate('student_id').populate('course_id');
...
export const getExamResultById = async (req: Request, res: Response) => {
    try {
        const examResult = await ExamResult.findById(req.params.id).populate('exam_id student_id course_id');
        if (!examResult) {
