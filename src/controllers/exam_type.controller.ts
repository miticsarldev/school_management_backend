...
export const createExamType = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Type d'examen créé avec succès", newExamType });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Erreur lors de la création du type d'examen", error });
  }
};
...
export const getAllExamTypes = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      message: "Erreur lors de la récupération des types d'examen",
      error,
    });
  }
};
...
export const getExamTypeById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Erreur lors de la récupération du type d'examen",
      error,
    });
  }
};
...
export const updateExamType = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const updatedExamType = await ExamType.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!updatedExamType) {
      return res.status(404).json({ message: "Type d'examen non trouvé" });
    ...
    res.status(500).json({
      message: "Erreur lors de la mise à jour du type d'examen",
      error,
    });
  }
};
...
export const deleteExamType = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Erreur lors de la suppression du type d'examen",
      error,
    });
  }
};
